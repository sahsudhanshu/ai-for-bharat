...
export default function TechnicalDetailsScreen() {
    ...
    return (
        <SafeAreaView style={styles.safe}>
            <Stack.Screen options={{ title: 'Technical Analysis', headerShown: true }} />
            <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
                    ...
                    <Card style={styles.imageCard} padding={0}>
                        {mlData.yolo_image_url ? (
                            ...
                            <View style={styles.placeholderBox}>
                                <Text style={styles.placeholderText}>YOLO output not available (offline mode)</Text>
                            </View>
                        )}
                    </Card>
                </View>
                ...
                {/* Detected Crops */}
                <View style={styles.section}>
                    <Text style={styles.sectionTitle}>🐟 Detected Fish ({crops.length})</Text>
                    {crops.map(([key, crop]: [string, any]) => (
                        <Card key={key} style={styles.cropCard} padding={SPACING.base}>
                            <View style={styles.cropHeader}>
                                <Text style={styles.cropTitle}>{key.replace('_', ' ').toUpperCase()}</Text>
                                <Text style={styles.yoloConf}>YOLO: {(crop.yolo_confidence * 100).toFixed(1)}%</Text>
                            </View>
                            ...
                            <View style={styles.classSection}>
                                ...
                                <View style={styles.classRow}>
                                    <View style={styles.classInfo}>
                                        ...
                                        <Text style={styles.classConf}>{(crop.species.confidence * 100).toFixed(1)}%</Text>
                                    </View>
                            ...
                            <View style={styles.classSection}>
                                ...
                                <View style={styles.classRow}>
                                    <View style={styles.classInfo}>
                                        <Text style={[
                                            styles.classLabel,
                                            { color: crop.disease.label === 'Healthy Fish' ? COLORS.success : COLORS.warning }
                                        ]}>
                                            {crop.disease.label}
                                        </Text>
                                        <Text style={styles.classConf}>{(crop.disease.confidence * 100).toFixed(1)}%</Text>
                                    </View>
